...
const Thumbnail: React.FC<ThumbnailProps> = ({ title, imageSrc, description, link }) => {
  ...
  return (
    <div className={`${styles.thumbnail} my-[100px]`}>
      <img src={imageSrc} alt={title} className={styles.thumbnailImage} />
      <h3 className={styles.thumbnailTitle}>{title}</h3>
      <p className={styles.thumbnailSummary}>{summary}...</p>
      <a href={link} className={`${styles.thumbnailLink} hover:animate-heartbeat`}>Lire l'article</a>
    </div>
  );
};
