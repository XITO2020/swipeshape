...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await executeQuery('SELECT * FROM programs;', []);
  if (error) throw error;
  return NextResponse.json({ programs: data }, { headers: corsHeaders() });
}
...
export async function POST(req: NextRequest) {
  ...
  if (error) throw error;
  return NextResponse.json({ program: data[0] }, { status: 201, headers: corsHeaders() });
}