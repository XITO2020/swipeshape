...
import ToastRounded from "../components/Toast";
...
export default function Home() {
  ...
  async function handlePurchase(programId: string, programName: string) {
    ...
    if (res.ok) {
      ...
      setToast({ message: `Achat réussi ! Un mail a été envoyé à ${session.user.email}.`, type: "success" });
      // Optionnel : envoyer le mail directement côté client, sinon l'API s'en charge
      // await sendProgramPurchaseEmail(session.user.email, programName, downloadUrl);
    } else {
  ...
  return (
    <>
      ...
      {toast && <ToastRounded message={toast.message} type={toast.type} />}
    </>
  );
}
